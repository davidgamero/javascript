...
export class KubeConfig implements SecurityAuthentication {
    ...
    private createDispatcher(
        ...
        // defaulting to 300s. Long-lived streams (watch, following logs, exec)
        // legitimately go quiet for longer than that, so it is disabled here.
        // Liveness comes from the TCP keepalive configured below, and watches
        // are additionally bounded by a server-side timeoutSeconds. headersTimeout
        // still applies, so a server that never responds is not held forever.
        //
        // The 'none' case deliberately keeps returning undefined so that a
        // globally installed dispatcher (such as MockAgent in tests) is used.
